...
RUN_AUTO_TAG = stash.pluginConfig['runAutoTag']
...
def start_library_monitor():
    ...
    def doIgnoreFileExt(chng_path, addToTargetPaths = False):
        ...
        if len(fileExtTypes) > 0:
            ...
            if suffix not in fileExtTypes:
                stash.Log(f"DEBUG: Ignoring file change because extension '.{suffix}' is not in monitored types {fileExtTypes}. Path='{chng_path}'")
                return True
        if len(excludePathChanges) > 0:
            for path in excludePathChanges:
                if chng_path_lwr.startswith(path.lower()):
                    stash.Log(f"DEBUG: Ignoring file change because path '{chng_path_lwr}' matches excluded path '{path}'.")
                    return True
        if addToTargetPaths:
    ...
    def on_created(event):
        global shouldUpdate
        stash.Log(f"DEBUG: on_created event received for '{event.src_path}' (is_directory={event.is_directory})")
        ...
        while True:
            ...
            if shm_buffer[0] != CONTINUE_RUNNING_SIG or StopLibraryMonitorWaitingInTaskQueue:               
                ...
                try:
                    ...
                    # unlink() internally calls resource_tracker.unregister, so nothing else needed here
                except FileNotFoundError:
                    # Already unlinked by another process; unlink() never called its internal
                    # resource_tracker.unregister, so do it manually to avoid "leaked objects" warning
                    try:
...
def stop_library_monitor():
    ...
    shm_a.close()
    try:
        shm_a.unlink()  # Remove the name so FileMonitor knows to stop
        # unlink() internally calls resource_tracker.unregister, so nothing else needed here
    except FileNotFoundError:
        # Already unlinked; unlink() never called resource_tracker.unregister, so do it manually
        try:
            from multiprocessing import resource_tracker
            resource_tracker.unregister(shm_a._name, "shared_memory")
        except Exception:
...
def start_library_monitor_service():
    stash.Log("start_library_monitor_service: checking if FileMonitor is already running.")
    ...
    try:
        ...
        pass
        stash.Log("FileMonitor is not running, so it's safe to start it as a service.")
    ...
    if JobIdOf_StartAsAServiceTask != None:
        ...
        stash.Log(f"start_library_monitor_service: will kill job ID {JobIdOf_StartAsAServiceTask} after launch.")
    if stash.API_KEY:
        args += ["-a", stash.API_KEY]
    stash.Log(f"start_library_monitor_service: launching script with args={args}")
    results = stash.executePythonScript(args)
    stash.Log(f"start_library_monitor_service: executePythonScript results='{results}'")
    if doJsonReturnFileMonitorStatus:
        time.sleep(5)
