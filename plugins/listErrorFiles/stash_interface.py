...
mutation SceneUpdate($input: SceneUpdateInput!) {
  sceneUpdate(input: $input) { id }
    ...
    def metadata_generate(self, options=None):
        """Queues a generate task (covers/previews/sprites force ffmpeg to
        actually read through each file, which is what surfaces decode
        errors for corrupted/truncated files). Returns the job ID."""
        default_options = {
            "covers": True,
            "sprites": True,
            "previews": True,
...
mutation MetadataGenerate($input: GenerateMetadataInput!) {
  metadataGenerate(input: $input)
        ...
        data = self._gql(query, {"input": options or default_options})
...
query FindJob($input: FindJobInput!) {
  findJob(input: $input) {
    ...
    error
        ...
        data = self._gql(query, {"input": {"id": job_id}})
